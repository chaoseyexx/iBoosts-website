...
import { fetchDisputes } from "@/app/(admin)/admin/actions";
...
export default async function AdminDisputesPage() {
    ...
    const investigatingDisputes = disputes.filter(d => d.status === "UNDER_REVIEW" || d.status === "AWAITING_RESPONSE").length;
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-white tracking-tighter drop-shadow-md">
                        Dispute <span className="text-rose-500">Center</span>
                    </h1>
                    <p className="text-[#8b949e] font-medium mt-1">Review and resolve marketplace disputes.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: "Open Disputes", value: openDisputes.toString().padStart(2, '0'), icon: AlertTriangle, color: "rose", bg: "rose-500/10", border: "rose-500/20" },
                    { label: "Investigating", value: investigatingDisputes.toString().padStart(2, '0'), icon: ShieldAlert, color: "amber", bg: "amber-500/10", border: "amber-500/20" },
                    { label: "Resolved (30d)", value: resolvedDisputes.toString().padStart(2, '0'), icon: CheckCircle2, color: "emerald", bg: "emerald-500/10", border: "emerald-500/20" },
                ].map((stat, i) => (
            ...
            <div className="relative group border border-[#30363d]/50 bg-[#161b22]/40 backdrop-blur-xl rounded-3xl overflow-hidden shadow-2xl">
                {disputes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24">
                        ...
                        <h3 className="text-white font-bold tracking-tight">No Active Disputes</h3>
                        <p className="text-[#8b949e] text-sm mt-1">All marketplace transactions are running smoothly.</p>
                    </div>
                ) : (
                            ...
                            <thead>
                                <tr className="bg-black/40 border-b border-[#30363d]/50">
                                    <th className="px-8 py-6">
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8b949e]">Dispute ID</span>
                                    </th>
                                    <th className="px-8 py-6">
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8b949e]">Reason</span>
                                    </th>
                                    <th className="px-8 py-6">
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#8b949e]">Parties</span>
                                    </th>
                                    <th className="px-8 py-6">
                                ...
                                {disputes.map((dispute) => (
                                    <tr key={dispute.id} className="group/row hover:bg-rose-500/5 transition-all duration-300">
                                        <td className="px-8 py-6">
                                            <div className="font-black text-white text-base tracking-tighter tabular-nums group-hover/row:text-rose-400 transition-colors">
                                                #{dispute.id.slice(-8).toUpperCase()}
                                            </div>
                                            <div className="text-[9px] text-[#8b949e] font-black tracking-widest uppercase opacity-40 mt-1">
                                                Order: #{dispute.orderId.slice(-8).toUpperCase()}
                                            </div>
                                            <div className="text-[9px] text-rose-500 font-bold mt-1 uppercase tracking-tight">
                                        ...
                                        <td className="px-8 py-6">
                                            ...
                                            <div className="text-[10px] text-[#8b949e] mt-1 line-clamp-1">{dispute.description}</div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-3 group/buyer">
                                                    ...
                                                    <span className="text-xs font-bold text-white group-hover/buyer:text-[#58a6ff] transition-colors">{dispute.buyer?.username || "Unknown"}</span>
                                                </div>
                                                <div className="flex items-center gap-3 group/seller">
                                                    <div className="h-5 w-5 rounded bg-white/5 border border-white/10 flex items-center justify-center text-[9px] font-black text-rose-500">S</div>
                                                    <span className="text-xs font-bold text-white group-hover/seller:text-rose-500 transition-colors">{dispute.seller?.username || "Unknown"}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 font-black text-white text-xl tracking-tighter tabular-nums">
                                            ${Number(dispute.order?.listing?.price || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className={cn(
                                                ...
                                                dispute.status === "OPEN" ? "bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-[0_0_20px_rgba(244,63,94,0.1)]" :
                                                    dispute.status === "UNDER_REVIEW" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" :
                                                        "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                                            )}>
                                                <div className={cn(
                                                    "h-2 w-2 rounded-full animate-pulse",
                                                    dispute.status === "OPEN" ? "bg-rose-400 shadow-[0_0_8px_rgba(251,113,133,0.5)]" :
                                                        dispute.status === "UNDER_REVIEW" ? "bg-amber-400 shadow-[0_0_8px_rgba(245,158,11,0.5)]" :
                                            ...
                                            <div className="flex items-center justify-end gap-3">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="bg-[#0d1117]/95 backdrop-blur-2xl border-[#30363d]/50 text-[#c9d1d9] rounded-2xl p-2 min-w-[200px] shadow-2xl overflow-hidden">
                                                        ...
                                                        <DropdownMenuLabel className="font-black text-[10px] uppercase tracking-widest text-[#8b949e] px-4 py-3 text-center border-b border-white/5 mb-1">Dispute Actions</DropdownMenuLabel>
